...
export default function SalesOrderFooter({ salesOrder }) {
  const { company, declaration, preparedBy, approvedBy, footerNote } = salesOrder;
      ...
      <section className="grid grid-cols-3 gap-4 bg-slate-50/70 px-4 pb-4">
        ...
        <div className="rounded-[22px] border border-slate-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#f8fafc_100%)] p-4 text-[11px] shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          ...
          <p>
            ...
            {salesOrder.salesOrder.paymentTerms}
          </p>
          <p>
            <span className="font-semibold text-slate-500">Delivery:</span>{" "}
            {salesOrder.delivery.deliveryTerms}
          </p>
          <p>
            <span className="font-semibold text-slate-500">Dispatch:</span>{" "}
            {salesOrder.delivery.dispatchMode}
          </p>
        </div>
