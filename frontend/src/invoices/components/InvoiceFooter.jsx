...
export default function InvoiceFooter({ invoice }) {
  const { company, bankDetails, declaration, preparedBy, verifiedBy, footerNote } = invoice;
    ...
    <>
      <section className="grid grid-cols-3 gap-4 bg-slate-50/70 px-4 pb-4">
        <div className="rounded-[22px] border border-slate-200 bg-white p-4 text-[11px] shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          <p className="mb-1 font-semibold uppercase tracking-[0.18em] text-blue-700">Company PAN</p>
          <p className="mb-3">{company.pan}</p>
          <p className="mb-1 font-semibold uppercase tracking-[0.18em] text-slate-500">Declaration</p>
        ...
        <div className="rounded-[22px] border border-slate-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#f8fafc_100%)] p-4 text-[11px] shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          <p className="mb-2 font-semibold uppercase tracking-[0.18em] text-emerald-700">Bank Details</p>
          <p>
            <span className="font-semibold text-slate-500">Bank Name:</span> {bankDetails.bankName}
          </p>
          <p>
            <span className="font-semibold text-slate-500">Account Number:</span> {bankDetails.accountNumber}
          </p>
          <p>
            <span className="font-semibold text-slate-500">Branch:</span> {bankDetails.branch}
          </p>
          <p>
            <span className="font-semibold text-slate-500">IFSC:</span> {bankDetails.ifsc}
          </p>
        </div>
        <div className="flex min-h-[190px] flex-col justify-between rounded-[22px] border border-slate-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#eff6ff_100%)] p-4 text-[11px] shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          <div>
            <p className="font-semibold uppercase tracking-[0.18em] text-blue-700">Authorized Signatory</p>
          </div>
          <div className="space-y-2 text-right">
            <p>
              <span className="font-semibold text-slate-500">Prepared By:</span> {preparedBy}
            </p>
            <p>
              <span className="font-semibold text-slate-500">Verified By:</span> {verifiedBy}
            </p>
