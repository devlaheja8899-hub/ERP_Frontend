...
import { amountToWords, calculateInvoiceTotals, formatCurrency } from "../utils/invoiceFormatters";
  ...
  return (
    <>
      <section className="grid grid-cols-[1fr_0.9fr] gap-4 bg-slate-50/70 px-4 pb-4">
        <article className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          ...
          <table className="w-full border-collapse text-[11px]">
            <tbody>
              <tr>
                <td className="border-r border-b border-slate-200 px-4 py-3 font-semibold text-slate-600">Taxable Value</td>
                <td className="border-b border-slate-200 px-4 py-3 text-right font-medium text-slate-900">{formatCurrency(totals.taxableValue)}</td>
              </tr>
              {invoice.taxes.map((tax) => (
                <tr key={tax.label}>
                  ...
                  <td className="border-b border-slate-200 px-4 py-3 text-right font-medium text-slate-900">
                    {formatCurrency((totals.taxableValue * tax.rate) / 100)}
                  </td>
                </tr>
              ))}
              <tr>
                <td className="border-r border-slate-200 px-4 py-3 font-semibold text-slate-600">Total Tax Amount</td>
                <td className="bg-blue-50 px-4 py-3 text-right text-[13px] font-bold text-slate-900">{formatCurrency(totals.totalTaxAmount)}</td>
              </tr>
            </tbody>
          </table>
        </article>
        <article className="overflow-hidden rounded-[22px] border border-slate-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#eff6ff_100%)] shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          ...
          <div className="text-[11px]">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              ...
              <span className="font-medium text-slate-900">{formatCurrency(totals.discount)}</span>
            </div>
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              ...
              <span className="font-medium text-slate-900">{formatCurrency(totals.roundOff)}</span>
            </div>
      ...
      <section className="mx-4 mb-4 rounded-[22px] border border-slate-200 bg-[linear-gradient(135deg,_#f8fafc_0%,_#dbeafe_100%)] px-5 py-4 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-blue-700">Amount in Words</p>
        <p className="mt-2 text-[13px] font-semibold text-slate-900">{amountToWords(totals.grandTotal)}</p>
      </section>
    </>
  );
}
