...
export default function InvoiceSummary({ invoice }) {
  ...
  // Check if billTo state belongs to Maharashtra
  const taxes = useMemo(() => {
    const state = invoice?.billTo?.state?.toLowerCase() || "";
    ...
    const isMaharashtra = maharashtraCities.some((city) =>
      state.includes(city)
    );
  ...
  const totalTaxAmount = useMemo(() => {
    return taxes.reduce(
      (acc, tax) => acc + (totals.taxableValue * tax.rate) / 100,
      0
    );
  }, [taxes, totals.taxableValue]);
            ...
            <tbody>
              ...
              {taxes.map((tax) => (
                <tr key={tax.label}>
                  ...
                  <td className="border-b border-slate-200 px-4 py-3 text-right font-medium text-slate-900">
                    {formatCurrency(
                      (totals.taxableValue * tax.rate) / 100
                    )}
                  </td>
                </tr>
              ))}
