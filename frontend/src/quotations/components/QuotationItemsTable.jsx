...
import { formatCurrency } from "../utils/quotationFormatters";
...
export default function QuotationItemsTable({ items }) {
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: "Date Code", accessor: "dateCode", align: "center" },
      { Header: "Pack Type", accessor: "packType", align: "center" },
  ...
  return (
    <section className="bg-white px-4 pb-4">
      <div className="overflow-hidden rounded-[22px] border border-slate-200 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <table {...getTableProps()} className="min-w-full border-collapse text-[10px]">
              ...
              <tr {...headerGroup.getHeaderGroupProps()} key={headerGroup.id}>
                {headerGroup.headers.map((column) => (
                  <th
                    ...
                    key={column.id}
                    className={`border-r border-b border-white/10 px-2 py-3 font-semibold tracking-[0.08em] ${
                      column.align === "right"
                        ? "text-right"
                        : column.align === "center"
                          ? "text-center"
          ...
          <tbody {...getTableBodyProps()}>
                ...
                <tr {...row.getRowProps()} key={row.id} className="odd:bg-white even:bg-slate-50/70">
                  {row.cells.map((cell) => (
                    <td
                      {...cell.getCellProps()}
                      key={cell.column.id}
                      className={`border-r border-b border-slate-200 px-2 py-3 align-top text-slate-700 ${
            ...
            <tr className="bg-slate-50 font-semibold text-slate-900">
              <td className="border-r border-slate-300 px-3 py-3 text-center" colSpan={6}>
                Total
              </td>
